<?php
    ...
    if (isset($_POST['user']) && isset($_POST['pass'])) {
        ...
        if (empty($user)) {
            ...
            // Valid Input and Password format

            $grabUser=$conn->prepare('Select username from account where username= ?');
            $grabUser->bind_param('s',$user);
            $grabUser->execute();

            $res=$grabUser->get_result();
            print_r($res);
            if($res->num_rows==0) {
                $error='Your username is not in the database';
            } else if(verif)
            echo("WORK");
            // $_SESSION['user'] = 'admin';
